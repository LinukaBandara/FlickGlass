    ...
    <>
      ...
      {open ? (
        <div className="fixed inset-0 z-[60] md:hidden flex flex-col">
          {/* solid sheet from top — no giant empty hole */}
          <div className="relative z-10 bg-[#0B0C10]/95 backdrop-blur-xl border-b border-white/10 pt-[max(0.75rem,env(safe-area-inset-top))] px-3 pb-4 shadow-[0_20px_60px_rgba(0,0,0,0.65)]">
            <div className="flex items-center justify-between mb-3 px-1">
              <span className="font-extrabold tracking-[0.12em] text-sm">
                FLICK<span className="text-amber-400">GLASS</span>
              </span>
              <button
                type="button"
            ...
            <nav className="flex flex-col gap-0.5">
              {links.map((l) => (
                <a
                  ...
                  href={l.href}
                  onClick={() => setOpen(false)}
                  className="block px-4 py-3.5 rounded-xl text-base font-medium text-white/90 hover:bg-white/5 active:bg-white/10"
                >
                  {l.label}
                </a>
              ))}
            ...
            <button
              ...
              className="mt-3 w-full rounded-xl bg-amber-400 text-[#0B0C10] text-sm font-semibold py-3.5"
            >
              My Watchlist
            </button>
          </div>
          {/* tap rest of screen to close */}
          <button
            type="button"
            className="flex-1 w-full bg-black/60"
            aria-label="Close menu"
            onClick={() => setOpen(false)}
          />
        </div>
